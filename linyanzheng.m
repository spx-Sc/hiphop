...
ti=1;
% Rs=R;
    sinr(ti)=10*log10((woo'*Rs*woo)/(woo'*Rjn*woo));
    sinr1(ti)=10*log10((w1'*Rs*w1)/(w1'*Rjn*w1));
    sinr2(ti)=10*log10((w5'*Rs*w5)/(w2'*Rjn*w5));
    snap=256;
...
% figure;
% hold on
%  plot(sita,10*log10(B1/max(B1)),'g:','LineWidth',1.0);
...
%             xer=ones(201,1)*theta(i);
%             plot(xer,yer);
%         end
%         
%         nin;
%          legend('MVDR','DL','SVM_E_S_B','SVM');
